...
function replace_git_repo() {
    ...
    local default_repo=$(cat ./stepdefs/jenkins-config/${file} | grep -oP '(?<=<url>).*?(?=</url>)')
...
function xml_to_vm() {
    for file in "${xml_file[@]}"; do
        scp -i /home/centos/mykeypair ./stepdefs/jenkins-config/${file} root@10.0.2.100:/home &> /dev/null
