...
上記例内容では、IPアドレスが`10.0.22.100`です。
...
```
$ ssh root@10.0.22.100 -i mykeypair
```
...
> ```
> $ ssh root@10.0.22.100 -i mykeypair
> Warning: Permanently added '10.0.22.100' (RSA) to the list of known hosts.
> [root@31zmj9fr ~]#
> ```
...
> [root@31zmj9fr ~]# exit
> logout
> Connection to 10.0.22.100 closed.
> ```
...
ロードバランサーのIPアドレスは、`10.0.22.100`です。
